...
### 3.2 컬러 시스템
산성교회 사이트 분석 기반 컬러 팔레트. **디자인 방향: 흰색·따뜻한 크림 배경 주도, 네이비는 버튼·강조 포인트에만 사용.**

| 용도 | 컬러 | 코드 | Tailwind 변수 |
|------|------|------|------|
| Primary (버튼·강조) | 딥 네이비 — 배경 금지 | `#1B2A4A` | `--primary` |
| Secondary | 화이트 | `#FFFFFF` | `--secondary` |
| Accent | 소프트 골드 | `#C9A96E` | `--accent` |
| Text (본문) | 다크 | `#1A1A1A` | `--foreground` |
| Text (보조) | 미디엄 그레이 | `#6B7280` | `--muted-foreground` |
| Background | 흰색 | `#FFFFFF` | `--background` |
| Section-warm | 따뜻한 크림 (짝수 섹션·배너) | `#FAF8F5` | `--section-warm` |
| Divider / Border | 라이트 그레이 | `#E0E0E0` | `--border` |

> **로고 적용 가이드:** 헤더는 항상 흰색 배경이므로 로고 원본(다크) 버전 사용. Primary 컬러(`#1B2A4A`)는 CTA 버튼, 링크 hover 등 포인트 강조에만 적용.
